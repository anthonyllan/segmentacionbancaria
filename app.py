    ...
    with col1:
        ...
        if st.button("🔍 Analizar Cliente", type="primary"):
            if datos is not None:
                ...
                # Visualización
                st.write("### 📍 Posición en el Mapa de Clusters")
                
                # Crear gráfico con el nuevo cliente
                datos_viz = datos_con_clusters.copy()
                nuevo_cliente_viz = pd.DataFrame({
                    'nombre': [nombre],
                    'saldoCuentaAhorro': [saldo],
                    'frecuenciaUsoMensual': [frecuencia],
                ...
                fig = px.scatter(
                    ...
                    y='frecuenciaUsoMensual',
                    color='cluster',
                    title="Posición del Nuevo Cliente",
                    opacity=0.6
                )
...
# Visualización Avanzada
    ...
    if datos is not None:
        ...
        # Tabla de centroides
        st.write("### 🎯 Centroides de los Clusters")
        centroides_df = pd.DataFrame(
            kmeans.cluster_centers_,
            columns=['Saldo (Normalizado)', 'Frecuencia (Normalizada)'],
